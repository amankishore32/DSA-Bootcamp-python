# DSA-Bootcamp-python

DSA using python in 2 months. From 0 -> Hero

### Day1: Big O Notation -> _O(N)_

---

1. **Time Complexity (TC)** - How time grows as input data set (N) grows.
   <br>
   **_O(1)_** - Execution time SAME no matter how large the data set is.<br>
   **_O(N)_** - Time gets doubled if data set doubles.<br>
   **_O(N^2)_** - Danger Zone. If data is doubled, time Quadruples.

2. **Space Complexity (SC)** - How much space is required/consumed for input data set (N).<br>
   **_O(1)_** - No extra space is required. <br>
   **_O(N)_** - Same Extra space as the required as the input. i.e if input is 1GB we need 2GB space to execute.

### Day2: Arrays & Memory Managment

---

1. **Concept** - Array are contiguous memory locations.
2. **Trade-Off** - Access takes O(1) time while insertion/deletion at start are O(N) (need to shift elements)
3. **Technique** - In-Place reversal, using 2 pointers approach to achive SC: O(1)

### Day3: Two Pointers

---

Optimized 2 pointers approach for 2 Sum. From O(N^2) -> O(N).<br>
Start/End Pointers style.

### Day4: Palindrome -> _Two Pointers_

---

Using 2 pointers check is a string is a valid Palindrome or not. Skipping all alphaNum characters.
